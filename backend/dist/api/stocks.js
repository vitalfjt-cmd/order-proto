...
exports.stocks = (0, express_1.Router)();
function getValuationMethod(db, storeId) {
    const row = db
...
exports.stocks.get("/store-stocks", (req, res) => {
    try {
        const storeIdRaw = typeof req.query.storeId === "string" ? req.query.storeId : "";
        const keywordRaw = typeof req.query.keyword === "string" ? req.query.keyword : "";
        const asOfRaw = typeof req.query.asOf === "string" ? req.query.asOf : "";
...
exports.stocks.get("/monthly-summary", (req, res) => {
    try {
        ...
        const valuationMethod = getValuationMethod(db_1.db, storeId);
        const sql = `
      WITH params AS (
        SELECT
          @storeId AS store_id,
    ...
    `;
        ...
        const rows = stmt.all({ storeId, month: rawMonth, method: valuationMethod });
        res.json({
            ok: true,
            storeId,
            month: rawMonth,
...
exports.stocks.get("/valuation-settings", (req, res) => {
    const storeId = String(req.query.storeId ?? "");
    if (!storeId)
        return res.status(400).json({ error: "storeId is required" });
    const method = getValuationMethod(db_1.db, storeId);
    res.json({ storeId, method });
});
// POST /stocks/valuation-settings  { storeId:"0002", method:"MOVING_AVG" }
exports.stocks.post("/valuation-settings", (req, res) => {
...
exports.stocks.post("/monthly-adjust", (req, res) => {
    try {
        const body = (req.body || {});
        ...
        const diffLines = inputLines
            ...
            const actual = ln.actualQty === null || ln.actualQty === undefined
                ? closing
                : Number(ln.actualQty);
    ...
    `;
        const deleteStmt = db_1.db.prepare(deleteSql);
        const insertSql = `
    ...
    `;
        const findLastUnitCostStmt = db_1.db.prepare(`
      ...
      LIMIT 1
    `);
        const insertStmt = db_1.db.prepare(insertSql);
        ...
        const tx = db_1.db.transaction((lines) => {
            ...
            for (const ln of lines) {
                ...
                if (!(Number.isFinite(unitCost) && unitCost > 0)) {
                    const r = findLastUnitCostStmt.get({ storeId, itemId: ln.itemId });
