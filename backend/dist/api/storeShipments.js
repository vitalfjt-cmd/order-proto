...
exports.storeShipments = void 0;
const express_1 = require("express");
const db_1 = require("../db");
const id_1 = require("../lib/id");
exports.storeShipments = (0, express_1.Router)();
// ===================================
// 移動可能品目（在庫>0のもの）: GET /store/shipments/movable-items
//   ?storeId=0002&q=0010 (qは任意: 品目ID前方 or 名称部分一致)
// ===================================
exports.storeShipments.get("/store/shipments/movable-items", (req, res) => {
    try {
        const storeId = req.query.storeId ? id_1.ID.store(String(req.query.storeId)) : "";
        if (!storeId)
...
// ==============================
exports.storeShipments.get('/store/shipments', (req, res) => {
    const storeId = req.query.storeId ? id_1.ID.store(String(req.query.storeId)) : '';
    if (!storeId) {
    ...
    const where = ['from_store_id = @storeId'];
    ...
    if (movementTypeRaw === 'TRANSFER' || movementTypeRaw === 'DISPOSAL') {
        ...
        params.movementType = movementTypeRaw;
    }
...
// ==============================
exports.storeShipments.get('/store/shipments/:id', (req, res) => {
...
exports.storeShipments.post('/store/shipments/save', (req, res) => {
    const body = req.body || {};
    const h = body.header;
    ...
    const fromStoreId = id_1.ID.store(String(h.fromStoreId || ''));
    const toStoreId = h.toStoreId ? id_1.ID.store(String(h.toStoreId)) : null;
    const movementType = String(h.movementType || '').toUpperCase();
    ...
    const lines = rawLines
        .map(l => {
        const itemId = id_1.ID.item(String(l.itemId || ''));
    ...
    const tx = db_1.db.transaction(() => {
        ...
        const latestCosts = db_1.db.prepare(`
      ...
      `).all(fromStoreId, ...itemIds);
        ...
        const missing = itemIds.filter((id) => {
            if (!costMap.has(id))
                return true; // ← これが一番大事（履歴が無い）
            const v = costMap.get(id);
            return v === null || v === undefined || !Number.isFinite(Number(v));
        });
        if (missing.length > 0) {
            const err = new Error("unit_cost_missing");
            err.status = 409;
            err.body = {
                ok: false,
...
// ===================================
exports.storeShipments.post('/store/shipments/confirm', (req, res) => {
    try {
        ...
        const tx = db_1.db.transaction((shipIds) => {
            if (shipIds.length === 0) {
          ...
          `)
            ...
            if (missing.length > 0) {
                ...
                return res.status(409).json({
                    ok: false,
                    error: "unit_cost_missing",
                    message: "単価/金額が欠損している明細があるため、確定できません。いったん保存し直すか、先に入庫を作ってください。",
                    itemIds: items,
                });
            }
            // 追加 ここまで
