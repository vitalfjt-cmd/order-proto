...
// // === 店舗マスタ（/stores） ===
// // 呼び出し元：
// //   - frontend/src/App.tsx（発注画面の「店舗」プルダウン／モーダル）
// //   - frontend/src/HistoryPage.tsx（発注履歴の検索条件）
// // 参照テーブル：stores
// // 役割：ID と名称だけのシンプルな店舗一覧を { stores: [...] } で返す。
// app.get("/stores", (_req, res) => {
//   try {
//     let rows: any[] = [];
...
// //       発注画面：ベンダーフィルタ表示・名称解決
// //   - frontend/src/HistoryPage.tsx
// //       履歴／CSV のベンダー名表示
// //   - frontend/src/vendor/VendorShipments.tsx
// //       ベンダー出荷 - 受注一覧の検索条件／モーダル
// //   - frontend/src/vendor/VendorEdit.tsx
// //       新規伝票ヘッダのベンダー選択モーダル
// //
// // 参照テーブル：
// //   - vendors があれば vendors
// //   - 無ければ item_prices の vendor_id DISTINCT
// //
// // 役割：{ id, name } の配列としてベンダー一覧を返す。
// //       name が NULL の場合は id をそのまま name として返す。
// app.get("/vendors", (_req, res) => {
//   try {
//     let rows: any[] = [];
...
// // === 納品先マスタ（/master/stores） ===
// // 呼び出し元：frontend/src/vendor/VendorEdit.tsx
// //   - ヘッダの「納品先ID」選択モーダル
// // 参照テーブル：stores
// // 役割：出荷編集画面用に、納品先候補（店舗）の ID／名称一覧をそのまま返す。
// //       今は全店舗を対象にしている（将来フィルタする場合はここで制御）。
...
// // === 品目マスタ（ベンダー別）===
// // エンドポイント: GET /master/vendors/:vendorId/items
// // 呼び出し元：frontend/src/vendor/VendorEdit.tsx
// //   - 明細行の datalist（品目コード入力補助）
// //   - 品目選択モーダル（取扱品のみに絞った一覧表示）
// //
// // 参照テーブル：
// //   - vendor_items（取扱関係）
// //   - items（品目マスタ）
// //
// // 役割：指定されたベンダー（vendorId）が扱う品目一覧を返す。
// //       各行に id, name, spec, unit, tempZone を含む。
// //       items.temp_zone カラムは tempZone にキャメルケース変換して返す。
// //
// // 備考：以前の /master/items（全件）は廃止済み／廃止予定。
// //       「そのベンダーの取扱品のみ」に絞ることで誤発注防止・一覧の見やすさを優先。
// app.get('/master/vendors/:vendorId/items', (req, res) => {
//   try {
//     const vendorId = String(req.params.vendorId || '').replace(/\D/g, '').padStart(6, '0');

//     const rows = db.prepare(`
//       SELECT DISTINCT i.id, i.name, i.spec, i.unit, i.temp_zone AS tempZone
...
//   }
// });
// // === 品目マスタ（/master/items：全件）===
// // 呼び出し元：VendorEdit.tsx / apiVendor.ts の listItems()
// //   - ベンダー別マスタ（/master/vendors/:vendorId/items）のフォールバック用。
// //   - 取扱未登録ベンダーの場合などに、全品目から選べるようにしている。
...
// //     - orders, order_lines
// // =======================================================================
// // 旧 /ordering/rules は /ordering/entry に統合済み（rules.perVendor に含む）
...
// // 出荷（shipments）API
// // 呼び出し元：
// //   - VendorShipments.tsx（一覧／確定／生成）
// //   - VendorEdit.tsx      （編集／新規／明細保存）
// // ======================


// // === 出荷：一覧検索（/shipments）===
// // 呼び出し元：frontend/src/vendor/VendorShipments.tsx
// // 使う画面：ベンダー出荷 - 受注一覧
// // クエリ：vendorId, destinationId, from, to
// // 参照テーブル：v_shipments（shipments + stores + vendors のビュー）
// // 役割：指定期間＋ベンダー＋納品先でヘッダ一覧を返す。
...
// // === 出荷：単票取得（/shipments/:id）===
// // 呼び出し元：VendorEdit.tsx の getShipment()
// // 使う画面：ベンダー出荷 - 受注編集
// // パスパラメータ：id = shipment.id
// // 参照テーブル：v_shipments, v_shipment_lines
// // 役割：ヘッダ＋明細をまとめて返す（編集画面初期表示）。
...
// // === 出荷：ヘッダ作成／Upsert（/shipments）===
// // 呼び出し元：VendorEdit.tsx の handleSave()（新規伝票）
// // ボディ：{ vendorId, destinationId, deliveryDate, destinationName? }
// // 更新テーブル：shipments
// // 役割：同じ vendorId × destinationId × deliveryDate があれば再利用、
// //       無ければ新規作成して id を返す。
...
// // === 出荷：明細の置換保存（/shipments/:id/lines/replace）===
// // 呼び出し元：VendorEdit.tsx の replaceLines()
// // パス：id = shipments.id
// // ボディ：{ lines: [{ itemId, orderedQty, shipQty, unitPrice, amount, ... }] }
// // 更新テーブル：shipment_lines
// // 役割：一度対象ヘッダの明細を全削除し、リクエストの行でまるごと入れ替える。
// //       → 画面で「削除」した行もここで反映される。
...
// // --- 状態変更（確定・取消） ---
// // === 出荷：確定（/shipments/confirm）===
// // 呼び出し元：VendorShipments.tsx の「確定」ボタン
// // ボディ：{ ids: string[] } （対象 shipment.id の配列）
// // 更新テーブル：shipments
// // 役割：対象伝票の status を 'confirmed' に変更する。
...
// // === 出荷：確定取消（/shipments/unconfirm）===
// // 呼び出し元：VendorShipments.tsx の「確定解除」ボタン
// // ボディ：{ ids: string[] }
// // 更新テーブル：shipments
// // 役割：対象伝票の status を 'open' に戻す。
...
// // --- 受注→出荷生成 ---
// // ========== プレビュー（DBに書かない） ==========
// // === 出荷：生成プレビュー（/shipments/generate/preview）===
// // 呼び出し元：VendorShipments.tsx の「生成プレビュー」ボタン
// // ボディ：{ from?, to?, vendorId?, destinationId?, asOf? }（未指定ならサーバ側でデフォルト）
// // 参照テーブル：orders, order_lines, vendor_weekly_rules
// // ロジック：AGG_SQL で集計のみ実行し、ヘッダ数・明細数だけ返す。
// // 戻り値：{ ok:true, countHeaders, countLines, nowHHmm }
...
// // === 出荷：本番生成（/shipments/generate）===
// // 呼び出し元：VendorShipments.tsx の「締切越え→出荷生成」ボタン
// // ボディ：{ from?, to?, vendorId?, destinationId?, asOf? } （/preview と同じ）
// // 参照テーブル：orders, order_lines, vendor_weekly_rules
// // 更新テーブル：shipments, shipment_lines
// // ロジック：AGG_SQL で集計した結果を shipments / shipment_lines に INSERT/UPSERT。
// // 戻り値：{ ok:true, headersAffected, linesAffected }
...
//   } catch (e:any) { res.status(500).json({ error: e.message }); }
// });
// // === 出荷：明細のみ取得（/shipments/:id/lines）===
// // （※今のフロントからは基本 getShipment() でまとめて取る想定）
// // 参照テーブル：v_shipment_lines
// // 役割：必要であれば明細だけを別途返す簡易API。
